...
object Types {
  ...
  case class ObjectKeyModel(key: String)
  case class ObjectModel(created: String,
                         modified: Long,
                         encrypted: Boolean,
                         etag: String,
                         key: String,
  ...
  case class OwnerModel(id: String, name: String)

  case class GranteeModel(`type`: String,
                          id: Option[String] = None,
                          name: Option[String] = None) {
    require(QSConstants.GranteeTypes.contains(`type`),
            """type must be one of "%s" """.format(
              QSConstants.GranteeTypes.mkString(",")))
    require(
      `type`.equals("group")
  ...
  case class ACLModel(grantee: GranteeModel, permission: String) {
    require(QSConstants.ACLPermissions.contains(permission),
            """permission must be one of "%s" """.format(
              QSConstants.ACLPermissions.mkString(",")))
  }
  ...
  case class CORSRulesModel(allowed_origin: String,
    ...
    require(
      allowed_methods.forall(QSConstants.CORSHttpMethods.contains(_)),
      """allowed_methods must be member(s) of "%s" """.format(
        QSConstants.CORSHttpMethods.mkString(","))
    )
  }

  case class PartModel(part_number: Int,
  ...
  case class ConditionModel(string_like: Option[StringModel] = None,
  ...
  case class StatementModel(id: String,
    ...
    require(
      QSConstants.StatementEffects.contains(effect),
      "StatementModel: value of effect must be one of \"%s\"".format(
        QSConstants.StatementEffects.mkString(","))
    )
    require(id != null && id.nonEmpty, "StatementModel: id can't be empty!")
    require(resource != null && resource.nonEmpty,
            "StatementModel: resource can't be empty!")
    require(user.fold[String](str => str, list => list.mkString("")).nonEmpty,
            "StatementModel: user can't be empty!")
  }
}
